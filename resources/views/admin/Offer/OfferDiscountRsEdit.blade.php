<div class="box-body">
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-6 col-md-6">
            <div class="form-group">
                <label>Discount (Rs.)<span class="asterisk red">*</span></label>
        ...
        <div class="col-xs-12 col-sm-6 col-md-6">
            <div class="form-group">
                ...
                <select class="form-control select2" style="width: 100%;" name="apply_to" id="apply_to">
                    ...
                    <option value="Specific Brand" {{ $Offer->apply_to == 'Specific Brand' ? 'selected="selected"' : '' }}>Specific Brand</option>
                    <option value="Specific Collection" {{ $Offer->apply_to == 'Specific Collection' ? 'selected="selected"' : '' }}>Specific Collection</option>
                    <option value="Minimum Order Subtotal" {{ $Offer->apply_to == 'Minimum Order Subtotal' ? 'selected="selected"' : '' }}>Minimum Order Subtotal</option>
                </select>
        ...
        <div class="col-xs-12 col-sm-6 col-md-6" id="append_apply_to">
            <div class="form-group">
                @if($Offer->apply_to == 'Specific Product')
                    ...
                    <select class="form-control select2" style="width: 100%;" name="product[]" id="product" multiple>
                        <option value="">Choose One</option>
                    @foreach($pro as $k=>$v)
                        <option value="{{$v->id}}" <?php if(in_array($v->id, explode(',', $Offer->product_id))){ echo 'selected="selected"'; } ?> >{{$v->product_name}}</option>
                    @endforeach
                    </select>
                    <small class="help-block apply_to_on"></small>

                <?php /* @elseif($Offer->apply_to == 'Specific Category')
                ...
                @elseif($Offer->apply_to == 'Specific Collection')
                    <label>Brand <span class="asterisk red">*</span></label>
                    <?php $home_collection =  \App\HomeCollection::where('status','Active')->get(); ?>
                    <select class="form-control select2" style="width: 100%;" name="specific_collection" id="specific_collection">
                        <option value="">Choose One</option>
                        @foreach($home_collection as $k=>$v)
                            <option value="{{$v->id}}" {{ $Offer->home_collection_id == $v->id ? 'selected="selected"' : '' }}>{{$v->collection_for}}</option>
                        @endforeach
                    </select>
                    <small class="help-block apply_to_on"></small>
